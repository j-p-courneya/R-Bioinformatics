...
variants <- read.csv("https://osf.io/a325n/download")
```
...
>> head(variants[-1, ]) #returns first 6 rows, not including the first row.
>> ```
>>
>> f.  
>>
...
>>
>> ```{r, echo = FALSE, eval = TRUE}
>> head(variants[,c("sample_id")]) #returns the first 6 rows of the smple_id column
>> ```
>>
>> i.  
>>
...
>>
>> ```{r, echo = FALSE, eval = TRUE}
>> head(variants$sample_id) #returns the first 6 values of the sample_id clumn as a vector
>> ```
>>
>> l.  
>>
...
>>
>> ```{r, echo = FALSE, eval = TRUE}
>> head(variants[variants$REF == "A",]) #returns first 6 rows in which the RF column has the value "A"
>> ```
>>
>{: .solution} 
{: .challenge}
